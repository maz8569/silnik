...
namespace GameEngine {
	FPcam::FPcam(float fov, float aspect, float zNear, float zFar, float sensitivity)
	{
		m_Fov = fov;
		m_Aspect = aspect;
		m_zNear = zNear;
	...
	void FPcam::UpdateOnMouseMovement(double xpos, double ypos)
	{
		ypos = -ypos;
	...
	void FPcam::SetPosition(const glm::vec3& position)
	{
		m_Position = position;
	...
	void FPcam::ChangePosition(const glm::vec3& position_increment)
	{
	...
	void FPcam::SetFront(const glm::vec3& front)
	{
		m_Front = front;
		RecalculateViewMatrix();
	}

	void FPcam::SetRotation(float angle)
	{
		m_Rotation = angle;
	...
	void FPcam::SetAspect(float aspect)
	{
		m_Aspect = aspect;
	...
	void FPcam::SetNearAndFarPlane(float zNear, float zFar)
	{
		m_zNear = zNear;
		m_zFar = zFar;
	...
	void FPcam::SetPerspectiveMatrix(float fov, float aspect_ratio, float zNear, float zFar)
	{
		m_Fov = fov;
	...
	void FPcam::RecalculateViewMatrix()
	{
		m_ViewMatrix = glm::lookAt(m_Position, m_Front + m_Position, m_Up);
	...
	void FPcam::RecalculateProjectionMatrix()
	{
